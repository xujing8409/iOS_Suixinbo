...
@interface TCShowLiveMsgTableViewCell : UITableViewCell
{
    ...
    __weak TCSoLiveMsg      *_msgItem;
}

- (void)config:(TCSoLiveMsg *)item;
...
@implementation LiveUIMessageView
...
- (void)insertMsg:(id<AVIMMsgAble>)item
{
    ...
    else
    {
        @synchronized(_liveMessages)
        {
            ...
            if (_liveMessages.count >= kMaxMsgCount)
            {
                TCSoLiveMsg *msg = [_liveMessages objectAtIndex:0];
                scrolloff -= [TCSoLiveMsg defaultShowHeightOf:msg inSize:CGSizeMake(self.bounds.size.width, HUGE_VALF)];
            ...
            CGFloat heigt = [TCSoLiveMsg defaultShowHeightOf:item inSize:CGSizeMake(self.bounds.size.width, HUGE_VALF)];
            scrolloff += heigt;
...
- (void)insertText:(NSString *)message from:(id<IMUserAble>)user isMsg:(BOOL)isMsg
{
    ...
    TCSoLiveMsg *item = [[TCSoLiveMsg alloc] initWith:user message:message];
    item.isMsg = isMsg;
...
- (void)insertCachedMsg:(AVIMCache *)msgCache
{
    ...
    if (_isPureMode)
    {
        ...
        {
            TCSoLiveMsg *item = [msgCache deCache];
            if (item)
            {
    ...
    else
    {
        ...
        while (msgCache.count > 0)
        {
            ...
            {
                [items addObject:item];
                heigt += [TCSoLiveMsg defaultShowHeightOf:item inSize:CGSizeMake(self.bounds.size.width, HUGE_VALF)];
            }
        }
...
{
    TCSoLiveMsg *item = [_liveMessages objectAtIndex:indexPath.row];
    return [TCSoLiveMsg defaultShowHeightOf:item inSize:CGSizeMake(self.bounds.size.width, HUGE_VALF)];
}
