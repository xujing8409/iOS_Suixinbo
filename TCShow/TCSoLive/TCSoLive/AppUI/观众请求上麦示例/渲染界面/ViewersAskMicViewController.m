...
@interface ViewersAskMicViewController ()

@end

@implementation ViewersAskMicViewController

- (void)addLiveView
{
    ViewersAskMicUIViewController *liveUI = [[ViewersAskMicUIViewController alloc] initWith:self];
    [self addChild:liveUI inRect:self.view.bounds];
    
    _liveView = liveUI;
}

//请求上麦S_4:在渲染界面中创建房间引擎
- (void)createRoomEngine
{
    if (!_roomEngine)
    {
        ...
        _roomEngine = [[TCSoMultiLiveRoomEngine alloc] initWith:(id<IMHostAble, AVMultiUserAble>)_currentUser enableChat:_enableIM];
        _roomEngine.delegate = self;
    }
}

//请求上麦S_5: 在渲染界面初始化消息句柄，并赋值给交互界面中定义的消息句柄
- (void)prepareIMMsgHandler
{
    if (!_msgHandler)
    {
        _msgHandler = [[TCSoMsgMutilHandler alloc] initWith:_roomInfo];
        _liveView.msgHandler = (TCSoMsgMutilHandler *)_msgHandler;
...
//请求上麦S_6:实现user窗口回调，在此回调中，分配小窗口资源(在这个回调中设置小窗口视频在主屏中的位置)
...
@implementation TCSoMultiLiveRoomEngine
