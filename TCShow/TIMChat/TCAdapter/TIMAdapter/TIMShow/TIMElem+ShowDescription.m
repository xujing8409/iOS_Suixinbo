...
@implementation TIMVideoElem (ShowDescription)
// 显示描述
- (NSString *)showDescriptionOf:(IMAMsg *)msg
{
    // 后面转成对应的描述信息
...
- (NSString *)showDescriptionOf:(IMAMsg *)msg
{
    ...
    if ([msg isTIMCallMsg])
    {
        NSString *callType = [msg.customCMD isVoiceCall] ? @"[语音电话]" : @"[视频电话]";
        NSString *action = nil;
        switch ([msg.customCMD msgType]) {
            case AVIMCMD_Call_Dialing:
                action = @"正在呼叫";
                break;
            ...
            default:
                DebugLog(@"msg.customCMD msgType = %d", (int)[msg.customCMD msgType]);
                break;
        }
    
        return [NSString stringWithFormat:@"%@%@", callType, action ? action : @""];
    }
...
- (BOOL)isCallElem
{
    AVIMCMD *cmd = [AVIMCMD parseCustom:self];
    if (cmd)
    {
        // 缺少陌生人互起操作
        if (cmd.msgType >= AVIMCMD_Call && cmd.msgType <= AVIMCMD_Call_AllCount)
        {
            return YES;
        }
    }
