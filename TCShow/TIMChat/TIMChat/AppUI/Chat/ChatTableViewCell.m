...
@implementation ChatCallTableViewCell

// 只创建，外部统一添加
- (UIView *)addElemContent
{
    _callText = [[UILabel alloc] init];
    _callText.textAlignment = NSTextAlignmentLeft;
    _callText.numberOfLines = 0;
    _callText.lineBreakMode = NSLineBreakByWordWrapping;
    _callText.userInteractionEnabled = YES;
    return _callText;
}
...
- (void)configContent
{
    ...
    _callText.font = [_msg textFont];
    _callText.textColor = [_msg textColor];
    
    TIMCustomElem *elem = (TIMCustomElem *)[_msg.msg getElem:0];
    _callText.text = [elem showDescriptionOf:_msg];
